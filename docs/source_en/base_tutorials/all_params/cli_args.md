...
The **feature switch is command-line only**: add `--response-anomaly` to the command to enable detection (omit it to disable). The `response_anomaly` entry in the config file carries only non-switch settings (`payload_retention`, `payload_storage`, etc.):
