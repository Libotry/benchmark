...
When `model_name` is not configured explicitly, the **directory basename** of the model directory (`model_path`, or the model `path` field) is used as the model name — the basename is the de-facto model name (e.g. `/home/Qwen3-30B-A3B` → `Qwen3-30B-A3B`) and matches the config generator's default. The model `abbr` is a task label unrelated to the served model and is **never** used as a fallback. When neither `model_name` nor a model directory is available (e.g. only the explicit msProbe resource paths are configured), the task fails fast at startup and asks for an explicit `model_name`, instead of silently running detection with a wrong model name.
