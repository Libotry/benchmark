...
| `response_anomaly` | Dict | 可选，msProbe 推理响应异常检测的模型级配置，包含 `model_name`（与 msProbe 的 mtype_config.json 名称一致；未配置时回退到模型 `abbr` 并告警）、`model_path`（本地模型目录，用于自动生成配置，可选）、`msprobe_config_path`（算法阈值 config.yaml 路径，可选，用于手工调优；自动生成不会覆盖已存在的文件）、`msprobe_mtype_path`、`msprobe_token2category_dir`。未提供 mtype/token 分类路径时回退到 msProbe 包内默认文件 |
